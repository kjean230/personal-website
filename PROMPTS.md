...
Not a session prompt; the standing answer to *what tooling a session may reach for*, recorded so the four remaining sessions inherit it instead of re-deriving it. It belongs here because `BUILD_PLAN.md` §5 already names this file as the home for "the session guidance, tooling and capability limits". It deliberately does **not** live in `CLAUDE.md`: the precedence paragraph every session block pastes confines that file to repo conventions — commands, code style, file layout — so posture placed there would be read as down-weighted the moment it disagreed with anything. The one piece of this that *is* a repo convention, the licence rule, is in `CLAUDE.md` and only there.

**Agents. Claude is the only agent, and the leader.** Subagents, workflows and deep-research are **off** unless the owner asks for them in that session. The reason is not cost: this project's unit of work is a sub-branch that ends at a PR (§2.1), and a fan-out inside one session produces work no handoff describes and no gate reviewed. `BUILD_PLAN.md` §2's "one agent per lane, never two agents on one lane branch" holds regardless of what any session turns on — it is a statement about branch ownership, not about concurrency inside a session. **Codex is not wired into this harness**; do not write a prompt that assumes it is.

**Gemini Pro as a second gaze.** The owner has it, and it is a **manual paste loop, not an integration** — no agent here can call it, so nothing in a session block may depend on its output arriving. Use it at **one checkpoint per sub-branch**: after the diff is complete and the local gate suite is green, **before** the PR opens. That placement is the whole value — early enough to change the diff, late enough that the diff is real. Because it has no repo context, **the ask must be self-contained**: paste the diff plus the *single* constraint to check it against — the token rule, the §7 security block, the 250,000 B script budget, the JS-off requirement — and never the bare instruction "review this", which against a repo it cannot see produces confident generic advice. Three rules bound it. Its output is **advice to verify against the brief and the plan, never authority**. A disagreement between it and an authority document is **resolved by the document**; if the disagreement turns out to be a genuine gap in the document, that is a §2.1 halt, not a judgment call to settle in the diff. And it is **never credited in a commit message, PR title or PR body** — the owner-only attribution rule covers any assistant, model or vendor without exception.

**Skills and plugins — posture, not installs.** Nothing below is installed today. This records what was evaluated and on what grounds, so the same five questions are not re-litigated four more times.

`anthropics/claude-plugins-official` — `/plugins` is first-party and is ordinary tooling. `/external_plugins` is third-party and is held to **the bar of a runtime dependency**, because that is what it is. The specific hazard is not hypothetical: any plugin shipping a `.mcp.json` grants that server ambient access to the working directory, and this working directory contains the git-ignored `.env` holding `SUPABASE_DB_PASSWORD`, `SUPABASE_ACCESS_TOKEN` and a hosted `DATABASE_URL` — production credentials for a database with real content in it. **Read the `.mcp.json` and the server source, or do not install it.**

`anthropics/skills` — first-party, and `./spec` plus `./template` are the reference to follow if a local skill is ever packaged for this repo. Its `docx`, `pdf`, `pptx` and `xlsx` skills are **source-available, not open source**, and must never be vendored into this MIT tree; the licence rule in `CLAUDE.md` states that as a convention. There is also a scope trap worth naming: the `pdf` skill invites a resume PDF, and `feat/recruiter-resume-print`'s plan row is `@media print` CSS plus the four settled contact values. A PDF export is scope absorbed mid-flight, which that session's Tripwire forbids — it is reported, not built.

`kepano/obsidian-skills` — **skipped.** Obsidian Flavored Markdown (wikilinks, embeds, callouts, properties) does not render on GitHub, and `handoff/*.md`, `CLAUDE.md`, `PROMPTS.md` and PR bodies are all read in the GitHub UI. The handoff family *is* the mechanism by which the next session gets its context, so a syntax that degrades there degrades the one thing this project cannot afford to lose. Its `defuddle` skill alone may be worth revisiting for `BUILD_PLAN.md` §9's verify-at-build-time terms pages when `lane/ingestion` opens — and in the same breath, note that defuddle strips clutter, which means it would strip exactly the stylesheet content the Pair 2 motion notes' "read a live page's CSS" technique depends on. Right tool, wrong job, two lines apart.

`multica-ai/multica` — **not for this project.** It duplicates in a hosted service what this repo already does in git: handoffs, `lane/` and `feat/` branches, the §7 review gate. Worse, its premise — agents that pick work up on their own, plus cron autopilots — is the deliberate opposite of §2.1, whose entire design is that a sub-branch stops at a PR and a human decides what happens next. The bottleneck here has never been agent throughput; it is the owner decisions tabulated in `BUILD_PLAN.md` §8. Revisit only if running two concurrent lanes actually becomes a babysitting problem.

`DietrichGebert/ponytail` — the closest fit of the five, because its ladder is the one genuinely additive rule nothing in this repo states today (see Simplicity, below). If it is installed: **user scope, not project scope**, so it never becomes a thing a fresh clone silently depends on; **`lite`, never `ultra`**; and the **`ponytail:` marker stays unused**. That last one is not fussiness. `## Deferred` in every handoff is this repo's single deferral ledger, and §2.1 requires that anything deferred be written down there. A comment marker harvested later by a slash command is a *parallel ledger*, and a parallel record is precisely how the `lane/spine` migration sentence in `CLAUDE.md` came to bind nobody for weeks — it was true somewhere and load-bearing nowhere. Two further facts to hold before installing: its subagent injection is **on by default**, which collides with the agents rule above and must be turned off; and its published benchmark measures greenfield feature work, so on convention-heavy work like print CSS and a sitemap derived from `SECTIONS` the honest expectation is **the low end of its own range**.

**Simplicity, the one rule worth adopting.** Nothing in the brief, the plan or `CLAUDE.md` says it today, and it is worth saying: no abstraction for single-use code, nothing speculative, no configurability that was not asked for, and no error handling for impossible states. The test to carry, because it is sharper than the Tripwire's "materially larger than its plan row" — **every changed line should trace directly to the request.** A line that cannot be traced is either scope that was absorbed instead of reported, or a guess about a future that has not been specified.

This is **guidance, not a new gate.** It does not override the §7 checklist, it adds no box, and it settles nothing the brief or the plan has already settled. It also explicitly does **not** apply to this repo's docblocks, which are a deliberate expense rather than an accident: `app/(explorer)/trophy.tsx` spends 25 lines on why `archived` renders as "Archived" and not "Locked", and `app/error.tsx` spends 40 on what the boundary does not cover. Both exist so a later session does not re-derive a decision that was already made carefully — which is the same reason this section exists. Prose that prevents a re-derivation is not the abstraction the rule is aimed at.

The rest of the behavioural guidance considered alongside it was **declined as duplication**, and is named here only so it is not proposed again: "stop when unclear" is §2.1's ambiguity trigger; "do not improve adjacent code" is §2.1's lane-ownership trigger plus every session's Tripwire; "mention dead code, do not delete it" is the `handoff/chore-spine-s8-closeout.md:31` precedent; and "do not invent content" is already in `CLAUDE.md` and brief §10. Each is already binding in a document that outranks this one.
