...
- **Commands** (npm):
  ...
  - CI (`.github/workflows/ci.yml`) runs lint, typecheck, test, tokens:check, build, and a Lighthouse budget (`lighthouserc.json`). The single required status check on `main` and `lane/*` is the aggregate job **`CI green`** — new CI jobs join its `needs` list.
