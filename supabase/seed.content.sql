...
values
  ...
  -- REVIEW: facet = research per BUILD_BRIEF §4.1 — confirm; summary (one line for the tile)
...
- Continuing into the AI Studio project in fall 2026, an industry-sponsored machine learning project delivered with a student team. Sponsor to be announced.$md$,
  ...
  -- REVIEW: facet (not in the export): corporate | research | volunteer | classroom | coursework, or leave null; summary (one line for the tile)
...
- Developed product-specific prompt endpoints, delivering the dental line first with the service designed to extend to additional coverage lines, and built polling APIs to track long-running generation jobs.$md$,
  ...
  ('4d136059-c59e-56b0-ba89-1c87c81c8e61',
   'experience',
   null,
   'fordham-university-undergraduate-research-assistant',
...
- Extended the graph with authentication and session nodes and exposed it through a FastAPI service with a read-only GraphQL layer, splitting relational auth data in MySQL from graph data in Neo4j.$md$,
  ...
  ('a0b8399f-2df4-5fde-a700-07bfd5eeee81',
   'experience',
   null,
   'fordham-university-teaching-assistant-for-information-and-data-management-cisc-2500',
...
- Supported course operations and technical communication.$md$,
  ...
  ('c209e95a-a6bb-5aef-b139-63525cec4be6',
   'experience',
   null,
   'wildlife-conservation-society-data-analyst-researcher',
...
- Transformed and standardized CSV data using Python libraries, enhancing the quality of visual analytics outputs for research interns.$md$,
   '2025-09-01',
   '2026-01-01',
   false,
   'unlocked',
  ...
  ('da225b84-fe5a-51cc-86e3-940662726a09',
   'experience',
   null,
   'wildlife-conservation-society-data-field-researcher',
...
- Authored a scientific poster presented at the American Museum of Natural History, showcasing research findings.$md$,
   '2025-06-01',
   '2025-08-01',
   false,
   'unlocked',
  ...
  ('3d7d6c07-6ba6-59da-ac8e-bf840933c78d',
   'experience',
   null,
   'fordham-university-instep-program-mentor',
...
- Organized enrichment activities to develop leadership, resilience, and problem-solving skills.$md$,
  ...
  ('82015acc-a08b-5e82-849a-ea2b00c9256d',
   'experience',
   null,
   'fordham-university-step-tutor-counselor',
...
- Provided individualized tutoring and academic support, significantly improving student performance in core STEM subjects.$md$,
  ...
  ('38d15ee0-5eee-55ef-91a6-57e6cac1175e',
   'experience',
   null,
   'fordham-university-cstep-summer-scholar',
  ...
  -- REVIEW: facet (not in the export): corporate | research | volunteer | classroom | coursework, or leave null; summary (one line for the tile); subtitle (organisation / issuer — not in the export)
  ('d7c5b422-bba3-5942-9347-2e1689dfa819',
   ...
   null,
   null,
   $md$Built linear regression models in Python and scikit-learn to test how temperature, air quality, season, and year relate to fly and spider abundance in field data collected around Pelham Parkway in the Bronx. Standardized continuous predictors and one-hot encoded seasonal categories, then exported per-taxon coefficients and predictions to CSV so results stayed reproducible. Wrote the visualization layer in Matplotlib and pandas to produce the figures I presented in a scientific poster at the American Museum of Natural History. Reported adjusted R-squared alongside R-squared, which showed the models explained little variance at this sample size and that the coefficients were not reliable enough to draw conclusions from.$md$,
   '2025-06-01',
   '2025-08-01',
   false,
   'unlocked',
  ...
  ('4da2b4fd-332a-5942-98bc-d7c1eea144ac',
   'education',
   null,
   'fordham-university-graduate-school-of-arts-and-sciences-master-of-science-ms',
  ...
  ('76cfe335-a6c2-5acc-95f0-7ae80b3dbf49',
   'education',
   null,
   'fordham-university-bachelor-of-science-bs',
...
Activities: Undergraduate Research Assistant (Database) Teacher's Assistant (CISC 2500) Black Student Alliance Computer Science Club CSTEP (Collegiate Science and Technology Entry Program) CSTEP Summer Scholar (Summer 2023)$md$,
  ...
  -- REVIEW: summary (one line for the tile); subtitle (organisation / issuer — not in the export)
  ('ac0c7640-e829-5908-859f-257fa9541bdd',
   ...
   null,
   null,
   $md$CSTEP awarded/supporting students like myself an academic scholarship endowed by a Fordham alumna and her husband$md$,
   '2023-11-01',
   null,
   false,
   'unlocked',
  ...
  -- REVIEW: summary (one line for the tile); subtitle (organisation / issuer — not in the export); start_date (not in the export)
  ('f7f7efa6-eb00-50a8-9662-1e6e8bc92113',
   ...
   false,
   0,
   '{"source":{"export":"linkedin","file":"Honors.csv"},"category":"award"}'::jsonb),
  ...
  -- REVIEW: summary (one line for the tile)
  ('27922083-51e7-514b-884b-4852c82b8383',
   'experience',
   'volunteer',
   'brooklyn-public-library-stocker',
   'Stocker',
   'Brooklyn Public Library',
   null,
   $md$-I assisted the workers with placing books and restocking supplies throughout my Freshman year of High school, up until the peak of the Coronavirus Pandemic$md$,
   '2019-06-01',
   '2020-06-01',
   false,
   'unlocked',
   ...
   false,
   0,
   '{"source":{"export":"linkedin","file":"Volunteering.csv"},"date_precision":"month","cause":"environment"}'::jsonb),

  -- credential: bgjKUexFfN
  -- REVIEW: title, issuer, and date were read from the credential page (owner-supplied credential URL; fields read from its public page) — confirm; summary (one line for the tile)
...
insert into public.entry_relations (from_entry_id, to_entry_id, relation_type) values
  -- REVIEW: airbnb-superhost-classifier part_of break-through-tech — the only project dated inside the Break Through Tech fellowship (Jul 2026) and its content (scikit-learn classifier) matches the Machine Learning Foundations curriculum named in the position
  ((select id from public.entries where slug = 'airbnb-superhost-classifier'),
   ...
   'part_of'),
  -- REVIEW: machine-learning-foundations certifies break-through-tech — owner-supplied credential URL; fields read from its public page
  ((select id from public.entries where slug = 'machine-learning-foundations'),
   (select id from public.entries where slug = 'break-through-tech'),
   'certifies')
on conflict do nothing;

-- Suggested by date overlap, not asserted (owner's call, BUILD_PLAN §8). Uncomment to enable.
-- insert into public.entry_relations (from_entry_id, to_entry_id, relation_type) values
--  -- SUGGESTED: urban-arthropod-abundance-modeling part_of wildlife-conservation-society-data-field-researcher — same dates (Jun–Aug 2025) and the position's poster at the American Museum of Natural History is the project's figure set
--  ((select id from public.entries where slug = 'urban-arthropod-abundance-modeling'),
--   (select id from public.entries where slug = 'wildlife-conservation-society-data-field-researcher'),
--   'part_of')
-- on conflict do nothing;
